...
const apiKey = 'AIzaSyDPP5jC6KhZPQs-dYyIMO6L7-N6JRIwnW0'; // Anahtar burada ✅
...
const generateMedicalContent = async (category, specificTopic = null) => {
  ...
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-001:generateContent?key=AIzaSyDPP5jC6KhZPQs-dYyIMO6L7-N6JRIwnW0`,
      {
        method: 'POST',
...
const generateQuizQuestion = async (topicTitle, topicContent) => {
  const prompt = `Konu: ${topicTitle}. İçerik: ${JSON.stringify(
    topicContent
...
const generateMedicalImage = async (prompt) => {
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-001:predict?key=AIzaSyAxckgkWk8VvTcYLWrdpl18VY2iYjJowNY`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          instances: [
            { prompt: `Medical illustration: ${prompt}. High detail.` },
          ],
...
const generateDifferentialDiagnosis = async (articleContent) => {
  const prompt = `Sen kıdemli bir Dahiliye hocasısın. Aşağıdaki klinik vakayı analiz et ve Harrison'a dayanarak en olası 3 ayırıcı tanıyı belirle. HTML listesi olarak döndür. Vaka: ${JSON.stringify(
    articleContent
  )}`;
  try {
    const response = await fetch(
...
const askGeminiAboutCase = async (articleContent, question) => {
  const prompt = `Sen bir Tıp hocasısın. Vaka: ${JSON.stringify(
    articleContent
    ...
    <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          >
            Dahiliye Asistanı Eğitim Platformu
          </p>
        </div>
      </div>
        ...
        {currentUser ? (
          <div className="flex items-center gap-3">
            ...
            <img
              ...
              alt="Profile"
              className="h-10 w-10 rounded-full border-2 border-white shadow-sm bg-gray-100 object-cover"
            />
            <button
            ...
            {currentUser?.role === 'admin' && (
              <div className="absolute top-4 right-6 flex gap-2 z-10">
                ...
                <button
                  ...
                  className="text-blue-600 bg-white rounded-lg p-1.5 shadow-sm border border-gray-200"
                  title="Düzenle"
                >
                  <Edit size={16} />
                </button>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">İçerik Üreticisi</h2>
          <p className="text-sm text-gray-500 mt-1">
            Yapay Zeka Destekli (Gemini Imagen)
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
        ...
        {isGenerating ? (
          <div className="py-8">
            ...
            <p className="text-blue-800 font-medium">
              Harrison taranıyor & Görsel çiziliyor...
            </p>
          </div>
        ) : (
          <>
            ...
            <button
              ...
              className={`px-6 py-3 rounded-lg font-medium flex items-center shadow-sm transition-all ${
...
export default function App() {
  ...
  const verifyAndCreateAccount = async (enteredCode) => {
    if (enteredCode === verificationCode) {
      ...
      try {
        ...
        await updateDoc(
          doc(db, 'artifacts', appId, 'public', 'data', 'users', docRef.id),
          { id: docRef.id }
        );
  ...
  const handleUpdateProfile = async (updatedData) => {
    ...
    try {
      await updateDoc(
        doc(db, 'artifacts', appId, 'public', 'data', 'users', currentUser.id),
        updatedData
      );
  ...
  const handleLike = async (id) => {
    ...
    await updateDoc(
      doc(db, 'artifacts', appId, 'public', 'data', 'articles', id),
      { likes: newLikes }
    );
  };
  ...
  const handleReply = async (articleId, commentId, text) => {
    ...
    await updateDoc(
      doc(db, 'artifacts', appId, 'public', 'data', 'articles', articleId),
  ...
  const handleVerifyContent = async (article) => {
    if (!currentUser || currentUser.role !== 'admin') return;
    await updateDoc(
      doc(db, 'artifacts', appId, 'public', 'data', 'articles', article.id),
  ...
  return (
    ...
    >
      <Header
        ...
        onLogoutClick={() => {
          setCurrentUser(null);
          localStorage.removeItem('harrison_user_id');
        }}
        isDarkMode={isDarkMode}
              ...
              <div className="flex gap-2 overflow-x-auto pb-4 mb-4">
                {CATEGORIES.map((c) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full whitespace-nowrap text-sm border ${
                      selectedCategory === c
